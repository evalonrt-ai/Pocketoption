...
def use_free_signal(user_id):
    u = get_user(user_id)
    sb.table("users").update({"free_used": u["free_used"] + 1}).eq("user_id", user_id).execute()

def activate_licence(code, user_id):
    code = code.strip().upper()
    res  = sb.table("licences").select("*").eq("code", code).execute()
    if not res.data:
        return False
    lic = res.data[0]
    if lic.get("used"):
    ...
    sb.table("licences").update({
        "used": True, "used_by": user_id, "used_at": datetime.now().isoformat()
    }).eq("code", code).execute()
    sb.table("users").update({
        "licensed": True, "licence_type": ltype,
        "licence_code": code, "expiry": expiry
